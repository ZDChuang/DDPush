...
public class ClientStatMachine {
	...
	public static ClientStatMachine newFromFile(long lastTick, boolean hasMessage0x10, long last0x10Time,
			long message0x11, long last0x11Time, int message0x20Len, long last0x20Time, byte[] message0x20) {
		ClientStatMachine csm = new ClientStatMachine();
	...
	private void push0x10() throws Exception {
		...
		if (this.lastAddr != null
				&& (System.currentTimeMillis() - this.lastTick) < 1000 * PUSH_IM_AFTER_ACTIVE_SECOND) {
	...
	public synchronized ArrayList<ServerMessage> onClientMessage(ClientMessage cm) throws Exception {
		...
		if (cm.getCmd() == ClientStatMachine.CMD_0x00) {// 心跳
			...
			System.out.println(DateTimeUtil.getCurDateTime() + ":heart beat " + (end - time) / 1000 + "seconds");
			time = end;
			...
			System.out.println("self define...");

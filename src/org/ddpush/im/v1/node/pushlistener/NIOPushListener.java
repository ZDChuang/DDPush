...
public class NIOPushListener implements Runnable {
	...
	public void initChannel() throws Exception {
		...
		System.out.println(key.interestOps() +" -- " + " -- "+ key.channel() + " -- "+ key.selector());
		System.out.println("NIO TCP Push Listener nio provider: " + selector.provider().getClass().getCanonicalName());
	}

	public void initExecutor() throws Exception {
		if (executor == null) {
			executor = new ThreadPoolExecutor(minThreads, maxThreads, 30, TimeUnit.SECONDS,
	 ...
	 * AtomicInteger cnt;
	 * 
	 * public test(AtomicInteger cnt) { this.cnt = cnt; }
	 * 
	 * public void run() { try { Socket s = new Socket("localhost",
	 * PropertyUtil.getPropertyInt("Constant.PUSH_LISTENER_PORT"));
	 * s.setSoTimeout(0); InputStream in = s.getInputStream(); OutputStream out =
	 * s.getOutputStream();
	 * 
	 * // for(int i = 600000; i< 700000; i++){ // while(true){ int key =
	 * cnt.addAndGet(1); if (key > 10000) { return; } out.write(1); out.write(1);
	 * out.write(16); out.write(StringUtil.md5Byte("" + key)); out.write(0);
	 * out.write(0); out.flush();
	 * 
	 * byte[] b = new byte[1]; // int read = in.read(b); System.out.println(b[0]);
	 * // } // while(true){ // int read = in.read(b); // System.out.println(b[0]);
	 * // if(read < 0){ // break; // } // } s.close(); System.out.println("bye~~");
	 * // long time = din.readLong(); // System.out.println("time:"+time); //
	 * din.readLong();
	 * 
	 * } catch (Exception e) { e.printStackTrace(); } } }
	 * 
	 * Thread[] worker = new Thread[10000]; AtomicInteger cnt = new
	 * AtomicInteger(-1); for (int i = 0; i < worker.length; i++) { Thread t = new
	 * Thread(new test(cnt)); worker[i] = t; } for (int i = 0; i < worker.length;
	 * i++) { worker[i].start(); try { Thread.sleep(2); } catch (Exception e) { } }
	 * 
	 * for (int i = 0; i < worker.length; i++) { try { worker[i].join(); } catch

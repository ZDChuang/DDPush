...
public class Receiver implements Runnable {
	...
	protected void processMessage() throws Exception {
		...
		System.out.println(new String(swap));

		ClientMessage m = new ClientMessage(address, swap);
		enqueue(m);
		System.out.println(
				"message:" + StringUtil.convert(m.getData()) + "----address:" + m.getSocketAddress().toString());

	}

	protected boolean enqueue(ClientMessage message) {
		boolean result = mq.add(message);
		System.out.println("---offer-----receive---address---" + message.getSocketAddress() + "---------");
		if (result == true) {
			queueIn.addAndGet(1);
		}
		return result;
	...
	protected ClientMessage dequeue() {
		...
		if (m != null) {
			System.out.println("---poll-----receive---address---" + m.getSocketAddress() + "---------");
